...
ITEMS_QUESTIONS_API_URL = f"{BASE_API_URL}/questions/search"
...
def api_items_questions(item_id):
    ...
    response = requests.get(f"{ITEMS_QUESTIONS_API_URL}", params)

    result = [
        {"question": q.get("text"), "answer": q.get("answer").get("text")}
        for q in response.json().get("questions", [])
        if (q.get("status").lower() == "answered")
    ]
